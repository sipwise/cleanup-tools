...
sub update_partitions {
    ...
    print "$table: min_ts: $min_ts max_ts: $max_ts\n";
...
SQL
        ...
        foreach my $part (keys %{$all_parts}) {
            ...
            if ($pdt < $dt_min) {
                ...
                die "Cannot remove partition $pname because it contains data!";
            }
        }
    } else {
        my $mpart = 'pmax';
...
sub backup_partition {
    ...
    unless (check_partition_exists($table, $pname)) {
        ...
        die "Cannot alter table: $mtable: ".$DBI::errstr if $DBI::err;
        $dbh->do("alter table $table exchange partition with table $mtable");
