...
sub _get_time_column {
	...
	# add other supported syntax here. eg. date(x), date_sub(x INTERVAL y day), etc.
	# raw column name:
...
sub archive_dump {
	...
	while (1) {
		...
		if ($vars{"archive-target"} ne '/dev/null') {
			my $target = $vars{"archive-target"} . "/$mtable." . sprintf('%04i%02i_%04i%02i%02i%02i%02i%02i', $y, $m, $now[5] + 1900, $now[4] + 1, @now[3,2,1,0]) . ".sql";
...
sub backup_table {
	...
	my $stop = sprintf('%04i-%02i-%02i', $range[1][0], $range[1][1], $range[1][2]);
	my $next;
	my $time = '00:00:00';
	while (($date cmp $stop) <= 0) {
		...
		#print "$date $time - $next $time: $total records moved from $table to $mtable\n";
	} continue {
		$date = $next;
	}
...
sub cleanup {
	...
	while (($date cmp $stop) < 0) {
		...
		#print "$date $time - $next $time: $total records deleted from $table\n";
	} continue {
		$date = $next;
	}
	$total = _delete_loop($table, $col, "$date $time", "$date $stop_time");
	#print "$date $time - $date $stop_time: $total records deleted from $table\n";
