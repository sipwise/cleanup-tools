...
sub backup_table {
	...
	foreach my $cmonth (($vars{"backup-retro"} - 1,0)) {
		my $tmonths = $cmonth + $vars{"backup-months"};
		my $t = $now - int(30.4375 * 86400 * $tmonths);
	...
	while (($date cmp $stop) <= 0) {
		...
		#print "$date $time - $next $time\n";
		_move_loop($table, $mtable, $col, "$date $time", "$next $time");
	} continue {
		$date = $next;
	}
...
sub cleanup {
	...
	my $t = time() - int(86400 * ($vars{"cleanup-days"} + 1));
	my @t = localtime($t);
	...
	while (($date cmp $stop) < 0) {
		$next = _add_days($date,1);
		print "$date $time - $next $time\n";
		_delete_loop($table, $col, "$date $time", "$next $time");
	} continue {
		$date = $next;
	}
	print "$date $time - $date $stop_time\n";
	_delete_loop($table, $col, "$date $time", "$date $stop_time");
